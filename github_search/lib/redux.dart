...
// The State represents the data the View requires.
// The View consumes a Stream of States.
// The view rebuilds every time the Stream emits a new State!
// The State Stream will emit new States depending on the situation:
// The initial state, loading states, the list of results,
// and any errors that happen.
// The State Stream responds to input from the View by accepting a Stream<String>.
// We call this stream the onTextChanged "intent".
class SearchState {
  final SearchResult result;
  final bool hasError;
  final bool isLoading;

  SearchState({this.result, this.hasError = false, this.isLoading = false});
...
/// fetch a new set of results. This ensures only results for the latest search term are shown
///
...
class SearchMiddleware implements MiddlewareClass<SearchState> {
  ...
  void call(Store<SearchState> store, dynamic action, NextDispatcher next) {
    ...
    // This will stop us from over-fetching from our backend.
    _timer = new Timer(new Duration(milliseconds: 250), () {
      ...
      // accidentally showing state results.
...
/// The Search Epic provides the same functionality as the Search Middleawre,
/// but uses redux_epics and the RxDart package to perform the work. It will
/// listen for Search Actions and perform the search after the user stop typing for 250ms.
